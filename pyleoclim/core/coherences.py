...
class Coherence:
    ...
    def __init__(
        ...
        if scale_unit is not None:
            ...
            self.scale_unit = plotting.infer_period_unit_from_time_unit(
                timeseries1.time_unit
            )
        elif timeseries2 is not None:
            self.scale_unit = plotting.infer_period_unit_from_time_unit(
                timeseries2.time_unit
            )
        else:
    ...
    def plot(
        ...
        if in_scale:
            ...
            if ylabel is None:
                ylabel = (
                    f"Scale [{self.scale_unit}]"
                    if self.scale_unit is not None
                    else "Scale"
                )

            if yticks is None:
                yticks_default = np.array(
                    [
                        0.1,
                        0.2,
                        0.5,
                ...
                mask = (yticks_default >= np.min(y_axis)) & (
                    yticks_default <= np.max(y_axis)
                )
        ...
        if var == "wtc":
            ...
            cont = ax.contourf(
                self.time, y_axis, self.wtc[:, mask_freq].T, levels=lev, **contourf_args
            )
        elif var == "xwt":
            cont = ax.contourf(
                self.time, y_axis, self.xwt[:, mask_freq].T, levels=11, **contourf_args
            )  # just pass number of contours
        else:
        ...
        if self.signif_qs is not None:
            ...
            if signif_thresh not in self.qs:
                ...
                print(
                    "Significance threshold {:3.2f} not found in qs. Picking the closest, which is {:3.2f}".format(
                        signif_thresh, self.qs[isig]
                    )
                )
            else:
            ...
            if var == "wtc":
                signif_coh = self.signif_qs[0].scalogram_list[
                    isig
                ]  # extract WTC significance threshold
                signif_boundary = (
                    self.wtc[:, mask_freq].T / signif_coh.amplitude[:, mask_freq].T
                )
            elif var == "xwt":
                signif_coh = self.signif_qs[1].scalogram_list[
                    isig
                ]  # extract XWT significance threshold
                signif_boundary = (
                    self.xwt[:, mask_freq].T / signif_coh.amplitude[:, mask_freq].T
                )
    ...
    def dashboard(
        ...
        wavelet_plot_kwargs = (
            {} if wavelet_plot_kwargs is None else wavelet_plot_kwargs.copy()
        )
        ...
        self.timeseries1.plot(
            ax=ax["ts1"], color=line_colors[0], **ts_plot_kwargs, legend=False
        )
        ...
        ax["ts2"] = ax["ts1"].twinx()
        self.timeseries2.plot(
            ax=ax["ts2"], color=line_colors[1], **ts_plot_kwargs, legend=False
        )
        ...
        if "cbar_style" not in wavelet_plot_kwargs:
            wavelet_plot_kwargs.update(
                {"cbar_style": {"orientation": "horizontal", "pad": 0.15, "aspect": 60}}
            )
    ...
    def signif_test(
        ...
        from ..core.series import (
            Series,  # This is necessary for the multiprocessing pickling process!!! DO NOT REMOVE!!!!!
        )
        ...
        new.signif_qs = []
        new.signif_qs.append(
            MultipleScalogram(scalogram_list=wtc_list)
        )  # Export WTC quantiles
        new.signif_qs.append(
            MultipleScalogram(scalogram_list=xwt_list)
        )  # Export XWT quantiles
    ...
    def phase_stats(self, scales, number=1000, level=0.05):
        ...
        * kappa,  an estimate of the Von Mises distribution's concentration parameter.
        ...
        if scales.max() > self.scale.max():
            warnings.warn(
                "Requested scale exceeds largest scale in object. Truncating to "
                + str(self.scale.max())
            )
        ...
        if scales.size == 1:
            ...
            if idx_hi >= idx_lo:
                raise ValueError(
                    "Insufficiently spaced scales. Please pick a single one, or a wider interval"
                )
                ...
                res = waveutils.angle_sig(
                    phase, nMC=number, level=level
                )  # assess significance
...
class GlobalCoherence:
    ...
    def signif_test(self, method="ar1sim", number=200, qs=[0.95]):
        ...
        wavelet_kwargs = {
            "freq": self.coh.frequency,  # pass the frequency axis directly
    ...
    def plot(
        ...
        fill_alpha=0.3,
        fill_color="grey",
        coh_plot_kwargs=None,
        savefig_settings=None,
        spectral_kwargs=None,
        legend=True,
        legend_kwargs=None,
        spec1_plot_kwargs=None,
        spec2_plot_kwargs=None,
    ):
        ...
        spec1_plot_kwargs = (
            {} if spec1_plot_kwargs is None else spec1_plot_kwargs.copy()
        )
        spec2_plot_kwargs = (
            {} if spec2_plot_kwargs is None else spec2_plot_kwargs.copy()
        )
