...
class TestSurrogateSeriesFromParam:
    ...
    @pytest.mark.parametrize("seed", [None, 42])
    @pytest.mark.parametrize(
        "time_pattern,settings",
        [("even", None), ("random", None), ("specified", {"time": np.arange(50)})],
    )
    def test_from_param_t0(
        self, method, noise_param, number, seed, time_pattern, settings
    ):
        """Test from_series method"""
        surr = pyleo.SurrogateSeries(method=method, number=number, seed=seed)
    ...
    def test_from_param_CN_t0(self, param, nsim=10, eps=0.1, seed=108):
    ...
    def test_from_param_CN_t1(self, nsim=10):
        ...
        with pytest.raises(ValueError):
            CN.from_param(
                param=[2.0, 3.0], length=200
            )  # check that this returns an error
    ...
    def test_from_param_specified_time(self, length, number=2):
        ...
        surr.from_param(param=[5, 1], time_pattern="specified", settings={"time": ti})
    ...
    def test_from_param_random_time_t2(
        self, number=2, tol=3, param=[[1, 2], [0.80, 0.20]]
    ):
        ...
        for ts in surr.series_list:
            ...
            assert all(np.isin(delta_t, [1.0, 2.0]))
