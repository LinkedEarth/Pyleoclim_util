...
class TestUISeriesInit:
    ...
    def test_init_year_time_name_CE(self, evenly_spaced_series, units):
        ...
        v = ts.value
        ts2 = pyleo.Series(
            time=t, value=v, verbose=False, time_name="year", time_unit=units
        )
...
class TestUISeriesMakeLabels:
    ...
    def test_make_labels_t0(self, gen_normal):
    ...
    def test_make_labels_t1(self, gen_normal):
    ...
    def test_make_labels_t2(self, gen_normal):
...
class TestUISeriesSpectral:
    ...
    @pytest.mark.parametrize(
        "spec_method", ["wwz", "mtm", "lomb_scargle", "welch", "periodogram", "cwt"]
    )
    ...
    @pytest.mark.parametrize(
        "dt, nf, ofac, hifac", [(None, 20, 1, 1), (None, None, 2, 0.5)]
    )
    def test_spectral_t4(self, pinkseries, dt, nf, ofac, hifac, eps=0.5):
        ...
        psd = ts.spectral(
            method="mtm",
            freq="lomb_scargle",
            freq_kwargs={"dt": dt, "nf": nf, "ofac": ofac, "hifac": hifac},
        )
        beta = psd.beta_est().beta_est_res["beta"]
        assert np.abs(beta - 1.0) < eps
...
class TestUISeriesClean:
    ...
    def test_clean(self, gen_normal):
...
class TestSeriesAnnualize:   # Written mostly by Claude AI
    """Test Series annualize method with various parameter combinations and edge cases"""
    
    def test_annualize_default_all_months(self):
        """Test annualization with default parameters (all months)"""
        ts = pyleo.utils.load_dataset("SOI")
        annual_ts = ts.annualize()
        
        # Check that result is annual resolution
        resolution = annual_ts.resolution().describe()['median']
        assert resolution >= 0.9  # Should be approximately 1 year
        
        # Check that time axis contains years
        assert len(annual_ts.time) > 0
        assert annual_ts.label.endswith("(annualized)")
    
    @pytest.mark.parametrize("months", [
        [6, 7, 8],  # JJA summer
        [12, 1, 2],  # DJF winter (no negatives)
        [3, 4, 5],   # MAM spring
        [9, 10, 11]  # SON autumn
    ])
    def test_annualize_seasonal_months(self, months):
        """Test annualization with seasonal month selections"""
        ts = pyleo.utils.load_dataset("SOI")
        annual_ts = ts.annualize(months=months)
        
        # Should produce fewer years due to seasonal filtering
    ...
    def test_annualize_min_res_threshold(self, min_res):
        """Test annualization with different resolution thresholds"""
        ts = pyleo.utils.load_dataset("SOI")
        
        if min_res == 1.0:
            # Should raise error for high threshold
            with pytest.raises(ValueError, match=re.escape("min_res must be in (0, 0.5); got 1.0")):
                ts.annualize(min_res=min_res)
        else:
            # Should work for reasonable thresholds
            annual_ts = ts.annualize(min_res=min_res)
            assert len(annual_ts.time) > 0
    
    def test_annualize_single_month(self):
        """Test annualization with single month"""
        ts = pyleo.utils.load_dataset("SOI")
        annual_ts = ts.annualize(months=[7])  # July only
        
        assert len(annual_ts.time) > 0
        # Should have roughly one value per year with July data
    
    def test_annualize_invalid_months_out_of_range(self):
    ...
    def test_annualize_invalid_months_type(self):
        """Test annualization with invalid month parameter types"""
        ts = pyleo.utils.load_dataset("SOI")
        
        with pytest.raises(ValueError, match="months must be a list or tuple"):
            ts.annualize(months="invalid")
    ...
    def test_annualize_no_data_for_months(self):
        ...
        # Create a series with limited time range
        ts = pyleo.utils.load_dataset("SOI")
        # Truncate to only have data from one season
        start_idx = 100
        end_idx = 110
        ts_short = pyleo.Series(
            time=ts.time[start_idx:end_idx],
            value=ts.value[start_idx:end_idx],
    ...
    def test_annualize_annual_data_error(self, gen_normal):
        ...
        with pytest.raises(ValueError, match=re.escape("Data appears to be too coarse (median resolution: 1.0 years >= 0.25) for meaningful averages.")):
            ts.annualize()
    
    def test_annualize_wrong_time_unit(self):
        """Test that annualization fails without 'Year' in time_unit"""
        ts = pyleo.utils.load_dataset("SOI")
        ts_bad = ts.copy()
        ts_bad.time_unit = "days"  # Remove 'Year' from time_unit
        
        with pytest.raises(ValueError, match="time_unit must contain 'Year'"):
            ts_bad.annualize()
    
    def test_annualize_retrograde_series(self):
        """Test that annualization fails on retrograde (decreasing) time series"""
        ts = pyleo.utils.load_dataset("SOI")
        # Reverse the time axis to make it retrograde
        ts_retro = ts.sort(ascending=False)
        
        with pytest.warns(RuntimeWarning, match="Series was retrograde; it has been sorted in ascending order so annualization runs properly"):
            ts_retro.annualize()
    ...
    def test_annualize_preserve_metadata(self, ds_name):
        """Test that annualization preserves important metadata"""
        ts = pyleo.utils.load_dataset(ds_name)
        annual_ts = ts.annualize()
        
        # Check that important attributes are preserved
        assert annual_ts.time_unit == ts.time_unit
        assert annual_ts.value_unit == ts.value_unit
        assert annual_ts.value_name == ts.value_name
        assert f"{ts.label} (annualized)" == annual_ts.label
    
    def test_annualize_time_axis_format(self):
        """Test that the output time axis has correct format"""
        ts = pyleo.utils.load_dataset("SOI")
        annual_ts = ts.annualize()
...
class TestUISeriesSegment:
    ...
    def test_segment_t0(self):
        ...
        assert (
            str(type(ts_seg))
            == "<class 'pyleoclim.core.multipleseries.MultipleSeries'>"
        )

    def test_segment_t1(self, gen_normal):
...
class TestUISeriesSlice:
    ...
    def test_slice(self, gen_normal):
...
class TestSel:
    ...
    def test_value(self, value, expected_time, expected_value, tolerance):
        ...
        expected = pyleo.Series(
            time=expected_time, value=expected_value, time_unit="years", verbose=False
        )
    ...
    def test_invalid(self):
        ts = pyleo.Series(
            time=np.array([1, 2, 3]), value=np.array([4, 6, 1]), time_unit="years"
        )
...
class TestUISeriesSummaryPlot:
    ...
    def test_summary_plot_t0(self, pinkseries):
        ...
        assert (
            ax["scal"].properties()["ylabel"] == period_label
        ), "Period label is not being passed properly"
        assert (
            ax["psd"].properties()["xlabel"] == psd_label
        ), "PSD label is not being passed properly"
        assert (
            ax["scal"].properties()["xlabel"] == time_label
        ), "Time label is not being passed properly"
        assert (
            ax["ts"].properties()["ylabel"] == value_label
        ), "Value label is not being passed properly"
...
class TestUISeriesCorrelation:
    ...
    def test_correlation_t0a(self, gen_ts, sig_method, number, eps=0.1):
    ...
    def test_correlation_t0b(self, gen_ts, sig_method, eps=0.1):
    ...
    def test_correlation_t1(self, sig_method, eps=0.5):
        ...
        corr_res_evenly = ts1_evenly.correlation(
            ts2_evenly, method=sig_method, number=2
        )
        r_evenly = corr_res_evenly.r

        ts1 = pyleo.Series(
            time=air_time_unevenly,
            value=air_value_unevenly,
            verbose=False,
            auto_time_params=True,
        )
        ts2 = pyleo.Series(
            time=nino_time_unevenly,
            value=nino_value_unevenly,
            verbose=False,
            auto_time_params=True,
        )

        corr_res = ts1.correlation(
            ts2, method=sig_method, number=2, common_time_kwargs={"method": "interp"}
        )
        r = corr_res.r
    ...
    def test_correlation_t2(self, gen_ts, stat, eps=0.2):
    ...
    def test_correlation_t3(self, gen_ts, eps=0.1, nt=10):
    ...
    def test_correlation_t5(self):
        ...
        rho = 0.6
        tcrit = 1.96  # use quantile of standard normal bc nu is so large
        # generate series whose correlation with ts1 should be close to rho:
        v2 = rho * ts.value + np.sqrt(1 - rho**2) * np.random.normal(
            loc=0, scale=1, size=len(ts.time)
        )
    ...
    def test_correlation_t6(self, gen_ts, sig_method, number=10, eps=0.1):
        """Test the various significance methods"""
        nt = 100
        rho = 0.4  # target correlation
        ts1 = gen_ts(nt=nt, alpha=1, random_seed=333).standardize()
...
class TestUISeriesCausality:
    ...
    def test_causality_t0(self, gen_ts, method, eps=1):
    ...
    @pytest.mark.parametrize("method", ["liang", "granger"])
    def test_causality_t1(self, gen_ts, method, eps=1):
        """Generate two series from a same basic series and calculate their correlation
...
class TestUISeriesOutliers:
    ...
    def test_outliers_t1(self, gen_ts, remove_outliers):
    ...
    def test_outliers_t2(self, gen_ts, method):
    ...
    def test_outliers_t3(self, gen_ts, keep_log):
    ...
    def test_outliers_t4(self, gen_ts):
        # Generate data
        ts = gen_ts()
        # Add outliers
        outliers_start = np.mean(ts.value) + 5 * np.std(ts.value)
        ...
        # Remove outliers
        ts_out = ts2.outliers(
            method="kmeans", settings={"nbr_clusters": 2, "threshold": 2}
        )

    def test_outliers_t5(self, gen_ts):
        # Generate data
        ts = gen_ts()
        # Add outliers
        outliers_start = np.mean(ts.value) + 5 * np.std(ts.value)
    ...
    def test_outliers_t6(self, gen_ts, LOF_param):
        # Generate data
        ts = gen_ts()
        # Add outliers
        outliers_start = np.mean(ts.value) + 5 * np.std(ts.value)
    ...
    def test_outliers_t7(self, gen_ts):
        # Generate data
        ts = gen_ts()
        # Add outliers
        outliers_start = np.mean(ts.value) + 5 * np.std(ts.value)
        ...
        # Remove outliers
        ts_out = ts2.outliers(
            method="kmeans", settings={"LOF": True, "n_frac": 0.8, "contamination": 0.1}
        )
...
class TestUISeriesGkernel:
    ...
    def test_gkernel_t1(self, gen_ts):
    ...
    def test_gkernel_t2(self, gen_ts):
    ...
    def test_gkernel_t3(self, gen_ts):
...
class TestUISeriesInterp:
    ...
    def test_interp_t1(self, gen_ts):
    ...
    def test_interp_t2(self, gen_ts):
        """Test the bin function by passing arguments"""
        ts = gen_ts(nt=550, alpha=1.0)
        # randomly remove some data pts
        n_del = 50
    ...
    def test_interp_t3(self, gen_ts, interp_method):
        """Test the interp function with default parameter values"""
        ts = gen_ts(nt=550, alpha=1.0)
        # randomly remove some data pts
        n_del = 50
...
class TestUISeriesDetrend:
    ...
    def test_detrend_t1(self, gen_ts, detrend_method):
        # Generate data
        ts = gen_ts(nt=550, alpha=1.0)
        # Add a trend
        slope = 1e-5
...
class TestUISeriesWaveletCoherence:
    ...
    def test_xwave_t0(self, gen_ts, xwave_method):
    ...
    def test_xwave_t2(self, gen_ts, mother):
    ...
    def test_xwave_t4(self, gen_ts):
        ...
        fmax = 10 * fmin
        scal = ts1.wavelet_coherence(
            ts2, method="cwt", freq_kwargs={"fmin": fmin, "fmax": fmax, "nf": nf}
        )
        freq = pyleo.utils.wavelet.freq_vector_log(
            ts1.time, fmin=fmin, fmax=fmax, nf=nf
        )

        assert all(scal.frequency == freq)

    def test_xwave_t5(self, gen_ts):
        """Test Series.wavelet_coherence() with WWZ with specified ntau"""
        ts1 = gen_ts(model="colored_noise")
        ts2 = gen_ts(model="colored_noise")
        _ = ts1.wavelet_coherence(ts2, method="wwz", settings={"ntau": 10})

    def test_xwave_t6(self, gen_ts):
    ...
    @pytest.mark.parametrize(
        "freq",
        [None, np.linspace(1 / 100, 1 / 2, num=20), "log", "nfft", "welch", "scale"],
    )
    def test_xwave_t7(self, gen_ts, freq):
...
class TestUISeriesGlobalCoherence:
    ...
    def test_globalcoherence_t0(self, gen_ts):
        """Test Series.global_coherence() with default arguments"""
        ts1 = gen_ts(model="colored_noise")
        ts2 = gen_ts(model="colored_noise")
        _ = ts1.global_coherence(ts2)

    def test_globalcoherence_t1(self, gen_ts):
    ...
    @pytest.mark.parametrize(
        "freq", [None, np.linspace(1 / 100, 1 / 2, num=20), "log", "nfft", "welch"]
    )
    def test_globalcoherence_t2(self, gen_ts, freq):
...
class TestUISeriesWavelet:
    ...
    def test_wave_t0(self, gen_ts, wave_method):
    ...
    @pytest.mark.parametrize("wave_method", ["wwz", "cwt"])
    def test_wave_t1(self, gen_ts, wave_method):
    ...
    def test_wave_t4(self, gen_ts, freq_meth):
    ...
    def test_wave_t5(self, gen_ts, freq):
...
class TestUISeriesSsa:
    ...
    def test_ssa_t1(self, gen_ts, trunc):
...
class TestUISeriesStripes:
    ...
    def test_stripes(self, gen_normal, show_xaxis):
...
class TestUISeriesHistplot:
    ...
    def test_histplot_t0(self, gen_normal, max_axis=5):
        ts = gen_normal()
    ...
    def test_histplot_t1(self, gen_normal, vertical=True):
        ts = gen_normal()
...
class TestUISeriesFilter:
    ...
    @pytest.mark.parametrize(
        "method", ["butterworth", "firwin", "lanczos", "savitzky-golay"]
    )
...
class TestUISeriesConvertTimeUnit:
    ...
    def test_convert_time_unit_t0(self, gen_ts, keep_log):
...
class TestUISeriesFillNA:
    ...
    @pytest.mark.parametrize(
        "timespan,nt,ts_dt,dt",
        [(None, 500, 8, 5), (None, 500, 1, 2), ([100, 400], 500, 4, 2)],
    )
...
class TestUISeriesSort:
    ...
    @pytest.mark.parametrize("keep_log", [True, False])
    def test_sort_t0(self, gen_ts, keep_log):
...
class TestResample:
    ...
    def test_resample_simple(self, rule, dataframe_dt, soi_metadata):
        ...
        ts = pyleo.Series.from_pandas(ser, soi_metadata)
        result = ts.resample(rule).mean()
        result_ser = result.to_pandas()
        expected_values = np.array([0.0, 1.0, 2.0, 3.0, 4.0])
    ...
    def test_resample_retrograde(self):
        ...
        expected = pd.Series(
            ...
            index=pd.DatetimeIndex(
                ["1952-07-01 12:00:00", "1951-07-02 12:00:00"], name="datetime"
            ).as_unit("s"),
        )
